...
public class MyTextGraphicsConverter implements TextGraphicsConverter{
    ...
    public String convert(String url) throws IOException, BadImageSizeException {
        ...
        if (maxRatio != 0){
            double ratio = img.getWidth() >= img.getHeight() ? img.getWidth() / img.getHeight() : img.getHeight() / img.getWidth();
            //System.out.println("w: " + img.getWidth() + " h: " + img.getHeight() + " ratio: " + ratio);
            if (ratio > maxRatio)
        ...
        if (newWidth > maxWidth || newHeight > maxHeight){
            double maxIndex = Math.max((double) newWidth / maxWidth, (double) newHeight / maxHeight);
            newWidth = (int) (newWidth / maxIndex);
            newHeight = (int) (newHeight / maxIndex);
            //System.out.println("w: " + newWidth + " h: " + newHeight + " maxIndex: " + maxIndex);
        }
        ...
        TextColorSchema schema = textColorSchema == null ? new MyTextColorSchema() : textColorSchema;
        char[][] textArray = new char[bwRaster.getHeight()] [bwRaster.getWidth()];
        //System.out.println("width: " + bwRaster.getWidth() + " height: " + bwRaster.getHeight());
        for (int h=1; h<bwRaster.getHeight(); h++) {
            for (int w=1; w<bwRaster.getWidth(); w++) {
        ...
        for (int i=0; i<textArray.length - 1; i++) {
            for (int j=0; j<textArray[0].length - 1; j++) {
                char s = textArray[i] [j];
                out.append(s);
                out.append(s);
            }
            out.append("\n");
